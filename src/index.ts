...
import { QQData, ModianResData } from './utils/types';
...
async function handleWsOnMessage(data: string): Promise<void>{
  ...
  const dataJson: QQData = JSON.parse(data);
  ...
  if(
    ...
    const resData: Array<ModianResData> = res.data.data;

    // 发送信息的模板
    const sum0: number = sum(
      resData[0].already_raised,
      resData[1].already_raised,
      resData[2].already_raised,
      resData[3].already_raised
    );

    const sum1: number = sum(
      resData[4].already_raised,
      resData[5].already_raised,
      resData[6].already_raised,
      resData[7].already_raised
    );
 ...
 * 臧聪：${ keepTwoDecimals(resData[7].already_raised) }`;
